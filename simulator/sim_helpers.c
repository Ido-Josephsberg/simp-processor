...
int write_diskout_file_wrapper(Simulator* sim, output_paths* paths) {
    ...
    if (write_disk_content_to_file(diskout_file, diskout_2D_arr) != 0) {
        printf("Error writing memory content to file %s\n", diskout_path);
        fclose(diskout_file);
        return -1; // Return error code
    }
...
int write_monitor_files_wrapper(Simulator* sim, output_paths* paths) {
    ...
    //*TODO: Implement the logic to extract the monitor_string from the simulator.
	//* TODO: check with iddo if the string will contain the line drops or not - I ASSUMED HERE IT DOES.
    char* monitor_string = "### Example leds string_need to implement real one"; // Placeholder for the actual string extraction logic

	//write the monitor string content to the txt file
    if (write_str_to_file(monitor_txt_file, monitor_string) != 0) {
        printf("Error writing memory content to file %s\n", monitor_txt_path);
        fclose(monitor_txt_file);
        return -1; // Return error code
    }
...
//TODO: need to check if writing into monitor yuv and monitor txt is the same or not.
    ...
    if (write_str_to_file(monitor_yuv_file, monitor_string) != 0) {
        printf("Error writing memory content to file %s\n", monitor_yuv_path);
